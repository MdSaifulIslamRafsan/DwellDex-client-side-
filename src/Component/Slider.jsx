...
const Slider = () => {
  ...
  const {sliderData} = useContext(FakeDataContext);
  ...
  const prevSlider = () =>
    setCurrentSlider((currentSlider) =>
      currentSlider === 0 ? sliderData.length - 1 : currentSlider - 1
    );
  const nextSlider = () =>
    setCurrentSlider((currentSlider) =>
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `url(${
          currentSlider === 0
            ? sliderData[sliderData.length - 1]?.img
            : sliderData[currentSlider - 1]?.img
        })`,
      }}
    >
      {/* arrow */}
      ...
      <div className="w-1/2 px-4 left-0 absolute drop-shadow-lg text-white rounded-lg">
        <h1
          ...
          className="lg:text-4xl text-md font-bold md:text-2xl mb-3"
        >
          {sliderData[currentSlider]?.estate_title}
        </h1>
        <p  data-aos="fade-right"
          data-aos-duration="1000"
          data-aos-easing="linear" className="text-xs sm:text-sm md:text-base lg:text-sm">
          {sliderData[currentSlider]?.description}
        </p>
      </div>
      {/* slider container */}
